...
def predict(path, adenine_list, other_list):
    ...
    valid_voxel, valid_label = load_valid_data(adenines, others, voxel_output, 1, 1)
    v_y = np_utils.to_categorical(valid_label, num_classes=2)

    mdl = load_model('./deepdrug3d.h5')
    # mdl.save_weights('./weights.h5')
    score = mdl.predict(valid_voxel)
    # score = []
    # ll=0
    # for i in range(len(valid_voxel)):
    #     sc = mdl.predict(valid_voxel[i].reshape(1,14,32,32,32))[0]
    #     score.append(sc)
    #
    #     print(str(i)+' '+str(sc))
    #     ll = ll+1
